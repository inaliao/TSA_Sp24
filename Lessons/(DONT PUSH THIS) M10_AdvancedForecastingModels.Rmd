...
df_frequency_spec <- data.frame(
  ...
  Frequency = c(1,2,12,52) #higher frequency dataset -> more granular-level data
)
...
* Seasonal ARIMA (can only model one seasonal pattern)
* State Space models
...
* ETS
* ARIMA with dynamic harmonic fourier components: non-seasonal ARIMA but add the full-year term as the exogenous variable
* TBATS
* Neural Network: allow the ARIMA to be non-linear (if neural network model has a better performance, can we further assume that the time sereis in non-linear?)
...
#if the file size is too large, do not push the file to git 
#or create a file to store the files that do not push to git
...
household_all <- read.table(unz(temp, "household_power_consumption.txt"),header=TRUE, sep=";") #unz: un-zip
unlink(temp) #remove the downloaded file 

#Fixing date columns and selecting active power only column
#convert to numeric 
act_power_all <- household_all %>% 
  mutate( Global_active_power = as.numeric(Global_active_power)) %>%  
  #create our own date columns 
...
act_power_hourly <- act_power_all %>% 
  filter( !is.na(Global_active_power)) %>% #filter the na value 
  group_by(Date,Year,Month,Day,Hour) %>% #group by hour 
  # here we left column with minutes out
...
#no NAs
#if we have NA after doing the aggregate average, then we have to do the cleaning 
```
...
```{r message=FALSE, warning=FALSE}
#use the hourly as an input for the msts 

ts_act_power_hourly <- msts(act_power_hourly$hourly_mean_active_power, 
                            seasonal.periods =c(24,168,8766), #specify hours?
                            start=c(2006,12,16))
...
#seasonal 365: daily seasonality 
```
...
#stlf: forecast each seasonal components 
#use training dataset 
#forecast horizon is 365 days (you can change the forecast horizon)
ETS_fit <-  stlf(ts_act_power_daily_train,h=365) 
#2006 to 2010 hourly data
...
ARIMA_Four_fit <- auto.arima(ts_act_power_daily_train, 
                             seasonal=FALSE, #ARIMA can not handle seasonal series
                             lambda=0, #no transformation 
                             xreg=fourier(ts_act_power_daily_train, 
                                          K=c(2,12)) 
                             #add external exogenous regressor to the model 
                             #to model weekly seasonality, we do not need 7 parameters
                             #we just need to parameters: weekday and weekend 
                             #to model the seasonality in a year, we don't need 365
                             #instead, 12 is enough to repeat the seasonal pattern 
                             #the less estimated parameters, the less bias of the model 
                             #play with the K to see if we can get a better fit 
                             )
#we are not doing any forecast, so we do not need to specify h
...
ARIMA_Four_for <- forecast(ARIMA_Four_fit,
                           xreg=fourier(ts_act_power_daily_train,
                                        K=c(2,12),
                                        h=365), #for the forier term)
                           h=365 #this is for the forecast function 
                           ) 
...
#the forecast error increases with time 
#because we are adding the standard deviation to the model 
```
...
NN_fit <- nnetar(ts_act_power_daily_train,p=1,P=0,xreg=fourier(ts_act_power_daily_train, K=c(2,12))) #P=0 -> non-seasonal 

#NN_for <- forecast(NN_fit, h=365) 
NN_for <- forecast(NN_fit, h=365,xreg=fourier(ts_act_power_daily_train, 
                                          K=c(2,12),h=365)) 
#xreg: forecast the external regressor
#K=c(2,12,24)
#12: monthly seasonality 
#24: daily seasonality 

#h in the xreg -> if the period is not specified, the forecast will feed the historical time period
#include another exogenous variable in the model 
...
```{r}
#call on the forecast value instead of the goodness of fit 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#check the RMSE and MAPE
#having multi-criteria approach to find the best model
#take the average value of the top two model as another model
#rolling forecast (instead of testing the performance in a specific year, expland the testing time window)
...
```{r}
#the time frame is based on the time frame of the first time series object in the autoplot
autoplot(ts_act_power_daily_test) +
  autolayer(ETS_fit, PI=FALSE, series="STL+ETS") +
  autolayer(ARIMA_Four_for, PI=FALSE, series="ARIMA + Fourier") +
  autolayer(TBATS_for,PI=FALSE, series="TBATS") +
...
*use different year but the same month 
