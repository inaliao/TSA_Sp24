...
```{r}
#difference between stl and decompose function 
#what will only work for multiplicative trend
#stl: seasonal trend 
#decompose: only take time series object 
...
#the random component is from the decompose function 
...
mean_inflow <- mean(na.exclude(inflow_random))  
#exclude NA or missing observation to compute mean and std
#using na.rm=TRUE or na.exclude()
#the missing value is caused by differencing the trend
...
#tsclean(): remove missing data and outleirs from the series, from the forecast package 

#if the outliers are not errors, then those outliers should be included in the anlaysis to represent the viability in the model
```
...
#remember to convert the type into numaric

#calculate how many NAs are there in the dataframe 
...
#na.omit: omit all NA in the dataframe
...
```{r}
#Just find the most extreme value
...
#keep removing the outliers until all the data points are within the red bandwidth
#or adjust based on the p-value (significant level)
```
