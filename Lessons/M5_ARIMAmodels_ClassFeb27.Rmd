...
electricity_price <- read.csv(here("Data/Average_retail_price_of_electricity_United_States_monthly.csv"),header=TRUE,skip=4)
...
#ideal: the residual should be white noise series 
#the mean of the residual is around 0
#no time dependent between the error term and time
#the coefficient is within the blue interval
#ar1:phi
#ma1: theta


#fitted: the estimate of the model 
#residual: estimated - observation 
...
#1 differencing order 
#1 number of moving average terms

Model_211 <- Arima(deseasonal_residential_price,order=c(2,1,1),include.drift=TRUE)
print(Model_211)
#coefficient interpretation
#coefficient in respect to yt and yt-1 
#coefficient in respect to yt-1 and yt-2 
#se is the standard error
#coefficient in respect moving average (ma1)
#drift -> intercept component 
...
#Let's check AIC
print(compare_aic)
#select the model with the lowest AIC: model_011
#no clear direction of the improvement 
```
...
#use max.D=0 and max.Q=0 to measure there is no seasonal components in the data 
#capital PQD is for seasonal series
#pqd is for non-seasonal series
...
#use the differencing series to detrend
...
autoplot(deseasonal_residential_price,series="Observed")+
  ...
  autolayer(Model_112$fitted,series="ARIMA(1,1,2)")+
  autolayer(Model_211$fitted,series="ARIMA(2,1,1)")+
  autolayer(Model_212$fitted,series="ARIMA(2,1,2)")

#the change of the fitted does not change significantly with the AR or MA components 
#if there is a spike in the after de-season -> use the seasonal ARIMA 
```
