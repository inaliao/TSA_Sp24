...
```{r}
SS_seas_test <- StructTS(ts_price,
                    type="BSM",fixed=c(NA,NA,NA,NA))  
 #this function has convergence issues
checkresiduals(SS_seas_test)

#fix the parameter manually
SS_seas <- StructTS(ts_price,
                    type="BSM",fixed=c(0.001,0.01,0.3,NA)) 

#this function has convergence issues
checkresiduals(SS_seas)
#stochastic linear trend still has high residuals and the trend in the series 
#we might need to differencing the series first 

SS_seas <- StructTS(ts_price,
                    type="BSM",fixed=c(0.1,0.01,0.3,NA)) 
checkresiduals(SS_seas)
#confidence interval increase -> start with the first paramter small (0.001)
...
#historical fit is the best-performance model 
#try another year to see if BSM is the best performed model 
